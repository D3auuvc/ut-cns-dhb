...
class VAE(torch.nn.Module):
    def __init__(self,ek_size,dk_size,channel,device=None):
        ...
        self.faceFC1 = torch.nn.Linear(512*5*4,5120).to(self.device)
        self.faceFCM = torch.nn.Linear(5120,256).to(self.device)
        self.faceFCV = torch.nn.Linear(5120,256).to(self.device)

        self.scrambleFC1 = torch.nn.Linear(512*5*4,5120).to(self.device)
        self.scrambleFCM = torch.nn.Linear(5120,256).to(self.device)
        self.scrambleFCV = torch.nn.Linear(5120,256).to(self.device)
        ...
        self.decoder =  torch.nn.Sequential(
            ...
            torch.nn.ConvTranspose2d(128,32,4,stride=2),       # 50
            torch.nn.ReLU(),
            ...
            torch.nn.ConvTranspose2d(32,3,4,stride=2)       #  50 (49*2=98+3=102)

        ).to(self.device)
        self.encoder.apply(weights_init)
        self.decoder.apply(weights_init)
        torch.nn.init.normal_(self.faceFC1.weight,mean=0.0, std=0.00001)
        torch.nn.init.normal_(self.faceFCM.weight,mean=0.0, std=0.00001)
        torch.nn.init.normal_(self.faceFCV.weight,mean=0.0, std=0.00001)

        torch.nn.init.normal_(self.scrambleFC1.weight,mean=0.0, std=0.00001)
        torch.nn.init.normal_(self.scrambleFCM.weight,mean=0.0, std=0.00001)
        torch.nn.init.normal_(self.scrambleFCV.weight,mean=0.0, std=0.00001)
    ...
    def reparametrize(self,mu,log_var):
        ...
        #z= z.type_as(mu)
        return mu + sigma*z
    def forward(self, face,scramble):
        facefeature = self.encoder(face)
        facefeature = facefeature.view(-1,512*5*4)
        facesampling = self.faceFC1(facefeature)
        facesampling = self.relu(facesampling)
        #print(f'sampling {torch.max(sampling)}')
        facemu = self.faceFCM(facesampling)
        facev = self.faceFCV(facesampling)
        #facev = self.relu(facev)
        faceout = self.reparametrize(facemu,facev)
        ...
        return faceout,facemu,facev,scrambleout,scramblemu,scramblev
