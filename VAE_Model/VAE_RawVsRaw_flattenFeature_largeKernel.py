...
class VAE(torch.nn.Module):
    def __init__(self,ek_size,dk_size,channel,device=None):
        ...
        self.FCM = torch.nn.Linear(512*2*10,256).to(self.device)
        self.FCV = torch.nn.Linear(512*2*10,256).to(self.device)
        ...
        self.decoder =  torch.nn.Sequential(
            ...
            #torch.nn.Conv2d(128, 128, dk_size, stride=1,padding=dk_size//2),
            #torch.nn.Conv2d(128, 128, dk_size, stride=1,padding=dk_size//2),   #  C=32,H=5,W=5
            torch.nn.ConvTranspose2d(128,512,(3,4),stride=3),        #   11
            torch.nn.ReLU(),
            torch.nn.Conv2d(512, 256, dk_size,stride=1,padding=dk_size//2 ),   #  C=64,H=11,W=15
            #torch.nn.Conv2d(256, 256, dk_size,stride=1,padding=dk_size//2 ),
            #torch.nn.Conv2d(256, 256, dk_size,stride=1,padding=dk_size//2 ),  # C=128,H=11,W=15
            torch.nn.ConvTranspose2d(256,128,(3,5),stride=(2,3)),      # 24
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, 64, dk_size,stride=1,padding=dk_size//2),   #  C=64,H=24,W=32
            #torch.nn.Conv2d(64, 64, dk_size,stride=1,padding=dk_size//2),
            #torch.nn.Conv2d(64, 64, dk_size,stride=1,padding=dk_size//2),    #  C=32,H=24,W=32
            torch.nn.ConvTranspose2d(64,32,(3,5),stride=(2,3)),       # 50
            torch.nn.ReLU(),
            torch.nn.Conv2d(32, 16, dk_size,stride=1,padding=dk_size//2),    #  C=16,H=50,W=97
            #torch.nn.Conv2d(16, 16, dk_size,stride=1,padding=dk_size//2),
            #torch.nn.Conv2d(16, 16, dk_size,stride=1,padding=dk_size//2),    #  C=16,H=50,W=97
            torch.nn.ConvTranspose2d(16,3,(2,3),stride=(2,3))       #  50 (49*2=98+3=102)
    ...
    def forward(self, face):
        ...
        facefeature = facefeature.view(-1,512*2*10)
